...
allProductLists.map( function (singleProduct) {
  ...
  <div class="productDesc">
    ...
    <div class="priveCart">
      ...
      <button data-productId="${singleProduct.id}" class="addCart">
        <svg
          width="35"
          height="34"
          viewBox="0 0 35 34"
...
const setCartCounter = (newCount) => localStorage.setItem("cartCounter", newCount);
...
const setCart = (productId, cartType = 'add') => {
  // collect all product added to cart
  let collectCart = getCart();
  // declare a variable called newCart without assigning a value to it
  let newCart;
  //  check cartType is to add or remove product from cart
  if(cartType == 'add'){
      // selecting one product from the data base by the index number
    newCart = allProductLists.filter((filterData) => filterData.id == productId);
  collectCart.push(newCart);

  }else{
    console.log('before', collectCart)
    let collectCartD = collectCart.findIndex((filterData) => filterData.id === parseInt(productId));
    collectCart.splice(collectCartD,1)
    console.log('index to be removed', typeof productId)
    console.log('after', collectCart)
  }

  // add the new product to the existing array list
  // set the collated cart list to local storage
...
for (let i = 0; i < allProductsAddToCart.length; i++) {
  allProductsAddToCart[i].addEventListener("click", function () {
    ...
    getCartCount ? setCartCounter(parseInt(getCartCount) + 1): setCartCounter(1);

    // Display the counter
    counterTag.textContent = getCartCounter();

    // Hide the add to caart button for the added product
    allProductsAddToCart[i].classList.add('hideTag');
    allProductsRemoveFromCart[i].classList.remove('hideTag');

    // Get the productId number of the clicked product
